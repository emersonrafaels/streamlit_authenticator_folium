...
def load_page_plan_estrategico():
    ...
    lista_mercados = list(st.session_state["df_planejamento"]["MERCADO"].unique())
    ...
    filtro_mercado = select_column1.selectbox(
        label="Mercado", options=lista_mercados, help="Selecione o mercado desejado"
    )

    filtro_regiao = select_column2.selectbox(
        label="Região", options=lista_regioes, help="Selecione a região desejada"
    )

    filtro_supt = select_column3.selectbox(
    ...
    for dado in st.session_state["df_planejamento"]:
        # Aplicar filtros
        if (
            (filtro_mercado == "Todos" or dado["MERCADO"] == filtro_mercado)
            and (filtro_regiao == "Todas" or dado["REGIÃO"] == filtro_regiao)
            and (filtro_supt == "Todas" or dado["SUPT"] == filtro_supt)
        ):
