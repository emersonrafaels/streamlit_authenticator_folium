...
from streamlit_authenticator.utils import generate_random_pw
...
class Authenticate:
    ...
    def login(self, form_name: str, location: str = 'main',
              form_name_username: str = 'Username',
              form_name_password: str = 'Password',
              form_name_button: str = 'Login',
              validator_insert_image=False,
              image=None,
              width_image=50,
              location_image='main',
              position_image='center') -> tuple:
        """
        ...
        if not st.session_state['authentication_status']:
            ...
            if not st.session_state['authentication_status']:
                ...
                if validator_insert_image:
                    if image is not None:
                        # INSERINDO LOGO NO AUTHENTICATOR
                        add_logo(logo_url=image,
                                 width=width_image,
                                 location=location_image,
                                 local_form=login_form,
                                 position_image=position_image)

                self.username = login_form.text_input(form_name_username).lower()
                st.session_state['username'] = self.username
                self.password = login_form.text_input(form_name_password, type='password')
